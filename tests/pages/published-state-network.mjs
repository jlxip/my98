...
for(const [name,type] of Object.entries({chromium,webkit})) {
 const f=await diskFixture({isolated:true}),server=await serveSite({headers:true}),browser=await type.launch();
 try {
  ...
  const hash=await p.evaluate(async()=>{
   ...
   await reader.commitState(prepared.token);if((await reader.read(10000,1))[0]!==42)throw Error('overlay');await reader.close();return hash;
  ...
  results.push({name,multiBlockBytes:stateBytes.length,cancellationAndRetry:true,hashVerified:true,cidCorruptionRejected:true,authenticationRejected:true,cli:name==='chromium'});console.log(results.at(-1));

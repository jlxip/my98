...
impl Io for Files {
    fn read(&self, source: &str, offset: u64, len: usize) -> Result<Vec<u8>> {
...
fn pack(source: &str, output: &str) -> serde_json::Value {
    ...
    while let Some(b) = e.next(&io).unwrap() {
    ...
    assert_eq!(hex::encode(e.verify_image(&io).unwrap()), hash);
    let mut cold = engine();
    io.bytes.set(0);
    io.calls.set(0);
    let start = Instant::now();
    cold.open(&io, output.into(), total).unwrap();
    cold.read(&io, 0, size.min(512) as usize).unwrap();
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("verify") => {
            let io = Files::default();
            let mut e = engine();
            e.open(&io, args[2].clone(), fs::metadata(&args[2]).unwrap().len())
                .unwrap();
            assert_eq!(
                hex::encode(e.verify_image(&io).unwrap()),
                file_hash(&args[3])
            );
        ...
        _ => {
            ...
            let results = vec![
                pack(raw, "build/disk/native-raw.slop86"),
                pack(fat, "build/disk/native-fat.slop86"),
            ];

export async function runRemoteUi(gateway) {
    ...
    $('user').value='disk fixtures';$('password').value='public compatibility password';$('login').requestSubmit();await idle();$('gateway').value=gateway;await click('remote');
    window.closeRemoteUi=()=>click('close');
    return checks;
}

...
try {for(const [name,type] of Object.entries({chromium,webkit})) {
 ...
 await writeFile(repo+`build/disk/browser-${name}.slop86`,Buffer.from(result.output));await writeFile(repo+`build/disk/browser-${name}.img`,Buffer.from(result.expected));delete result.output;delete result.expected;
 const downloadEvent=page.waitForEvent('download');await page.evaluate(()=>{const a=document.createElement('a');a.href=URL.createObjectURL(window.fatOutput);a.download='fat.slop86';a.click();});const download=await downloadEvent;await download.saveAs(repo+`build/disk/browser-${name}-fat.slop86`);
 ...
 execFileSync(repo+'build/disk-target/release/examples/compat',['verify',`build/disk/browser-${name}-fat.slop86`,`build/disk/browser-${name}-fat.img`],{cwd:repo,stdio:'inherit'});
 ...
 execFileSync(repo+'build/disk-target/release/examples/compat',['verify',`build/disk/browser-${name}.slop86`,`build/disk/browser-${name}.img`],{cwd:repo,stdio:'inherit'});
